...
# Check project id
If (-Not (Select-String -Path $Path -Pattern "com.crt.advproject.config.exe.debug.802174434" -SimpleMatch -Quiet)) 
{
...
$target = ((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Include paths (-I)"})

# Delete all color format dependents include paths if them exist
$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Include paths (-I)"}).listOptionValue|where {$_.value -eq "../../../../PlatformPackage/RTE"})
...
switch($ColorFormat)
{
    ...
    2 {$addAtt.Value = "&quot;`${ProjDirPath}/../../../../PlatformPackage/RGB565&quot;"}
    3 {$addAtt.Value = "&quot;`${ProjDirPath}/../../../../PlatformPackage/LumA44&quot;"}
    4 {$addAtt.Value = "&quot;`${ProjDirPath}/../../../../PlatformPackage/Index8&quot;"}
...
$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Defined symbols (-D)"}).listOptionValue|where {$_.value -eq ""})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Defined symbols (-D)"}).listOptionValue|where {$_.value -eq "EW_FRAME_BUFFER_COLOR_FORMAT=EW_FRAME_BUFFER_COLOR_FORMAT_RGBA8888"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Defined symbols (-D)"}).listOptionValue|where {$_.value -eq "EW_FRAME_BUFFER_COLOR_FORMAT=EW_FRAME_BUFFER_COLOR_FORMAT_RGB565"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Defined symbols (-D)"}).listOptionValue|where {$_.value -eq "EW_FRAME_BUFFER_COLOR_FORMAT=EW_FRAME_BUFFER_COLOR_FORMAT_LumA44"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Defined symbols (-D)"}).listOptionValue|where {$_.value -eq "EW_FRAME_BUFFER_COLOR_FORMAT=EW_FRAME_BUFFER_COLOR_FORMAT_Index8"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }
...
switch($ColorFormat)
{
    ...
    2 {$addAtt.Value = "EW_FRAME_BUFFER_COLOR_FORMAT=EW_FRAME_BUFFER_COLOR_FORMAT_RGB565"}
    3 {$addAtt.Value = "EW_FRAME_BUFFER_COLOR_FORMAT=EW_FRAME_BUFFER_COLOR_FORMAT_LumA44"}
    4 {$addAtt.Value = "EW_FRAME_BUFFER_COLOR_FORMAT=EW_FRAME_BUFFER_COLOR_FORMAT_Index8"}
...
$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Defined symbols (-D)"}).listOptionValue|where {$_.value -eq "EW_SURFACE_ROTATION=0"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Defined symbols (-D)"}).listOptionValue|where {$_.value -eq "EW_SURFACE_ROTATION=90"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Defined symbols (-D)"}).listOptionValue|where {$_.value -eq "EW_SURFACE_ROTATION=180"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU C Compiler"}).option|where {$_.name -eq "Defined symbols (-D)"}).listOptionValue|where {$_.value -eq "EW_SURFACE_ROTATION=270"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }
...
$target = ((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU Linker"}).option|where {$_.name -eq "Library search path (-L)"})
...
######################### LIBARIES ############################
# Select libary node
$target = ((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU Linker"}).option|where {$_.name -eq "Libraries (-l)"})

# Delete rte include
$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq "ewrte-m4-gcc"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

# Delete all libary includes if them exist
$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq ""})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq "ewgfx-m4-gcc"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq "ewgfx-m4-r90-gcc"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq "ewgfx-m4-r180-gcc"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "NXP MCU Tools"}).tool|where {$_.name -eq "MCU Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq "ewgfx-m4-r270-gcc"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }
...
######################### EXCLUDES FROM BUILD ############################
$target = (((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).sourceEntries.entry).excluding
...
switch($ColorFormat)
{
    ...
    2 {$NewColorFormat = "RGB565"}
    3 {$NewColorFormat = "LumA44"}
    4 {$NewColorFormat = "Index8"}
...
if($CurrentColorFormat -ne $NewColorFormat)
{
    ...
    (((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.crt.advproject.config.exe.debug.802174434"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).sourceEntries.entry).excluding = $Newtarget
}
